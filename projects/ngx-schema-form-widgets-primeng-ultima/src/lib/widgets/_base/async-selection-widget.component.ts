...
import { WidgetComponentHttpApiService } from ".."
...
export abstract class AsyncSelectionWidgetComponent extends NoHelperTextSpacer implements OnInit, OnDestroy {
  ...
  get createOptionsModel() {
    if (this.requiresRecreateOptionsmodel()) {
      ...
      this.selectOptions = [].concat(__selectOptions)
    }
