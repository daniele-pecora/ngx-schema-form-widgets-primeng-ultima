import { Component, OnInit, Pipe, PipeTransform, Renderer2, AfterViewInit, ViewChild, ElementRef } from '@angular/core'
import { ControlWidget, FormProperty, LogService } from 'ngx-schema-form'
...
@Component({
  ...
  styleUrls: ['./_fix_file_widget_primeng.css', './file.widget.scss', '../_component-helper/no-helpertext-spacer.widget.scss'],
...
export class FileWidgetComponent extends NoHelperTextSpacer implements OnInit, AfterViewInit {
  ...
  get imageRules() {
    return this.schema.widget.imageRules
  }
  get invalidImageDimensionsMessageSummary() {
    return this.schema.widget.invalidImageDimensionsMessageSummary
  }
  get invalidImageDimensionsMessageDetail() {
    return this.schema.widget.invalidImageDimensionsMessageDetail
  }
  /**
  ...
  testContentSecurityPolicy() {
    document.addEventListener("securitypolicyviolation", (e) => {
      console.log('*****', e.blockedURI); // ******* data
      console.log('*****', e.violatedDirective); // ******* img-src
      console.log('*****', e.originalPolicy); // ******* img-src 'self';
    })
  }
  ...
  async onUpload(event) {
    await this.doUpload(event, true)
  }
  ...
  async doUpload(event, propagateSuccesMsg: boolean) {
    ...
    const filterInvalidFiles = await this.checkFileValidity(event.files)
  ...
  async checkFileValidity(files) {
    const accept = this.schema.widget.accept
    ...
    for await (const file of filesToFilter) {
      ...
      if (!matchingFileType) {
        msgs.push({
          severity: 'error',
          summary: (this.fileUploadEl.invalidFileTypeMessageSummary || '').replace('{0}', file.name),
          detail: (this.fileUploadEl.invalidFileTypeMessageDetail || '').replace('{0}', accept)
        })

        removeFile(file, this.fileUploadEl.files)
        removeFile(file, this.fileUploadEl._files)
        this.fileUploadEl.focus = false
      } else {
        let addFile = true
  ...
  updateUploadButtonTitle() {
    ...
    if (input) {
      ...
      if (label) {
        ...
        if (this.schema.widget.uploadButtonTitle) {
          this.renderer2.setAttribute(input, 'aria-label', this.schema.widget.uploadButtonTitle)
          this.renderer2.setAttribute(input, 'title', this.schema.widget.uploadButtonTitle)
        } else {
